...
class TSP(object):
    ...
    def plot_iteration_curve(self):
        tsp = self
        tsp.dw.plt.figure()
        tsp.dw.plt.plot(tsp.iteration_numbers, tsp.iteration_distances, marker='o')
        tsp.dw.plt.title('算法迭代曲线')
        tsp.dw.plt.xlabel('迭代次数')
        tsp.dw.plt.ylabel('路径距离')
        tsp.dw.plt.show()
    ...
    def __init__(self, c_rate, m_rate, pop_size, ga_num):
        ...
        self.ga_num = ga_num
        self.iteration_distances = []  # 存储每次迭代后的最优距离
    ...
    def evolution(self):
        ...
        for i in range(self.ga_num):
            ...
            tsp.iteration_distances.append(tsp.best_dist)
            tsp.iteration_numbers.append(i + 1)
...
def main():
    tsp = TSP(0.7, 0.1, 100, 500)
    tsp.init()
    tsp.evolution()
    tsp.re_draw()
    tsp.dw.plt.show()
    tsp.plot_iteration_curve()
